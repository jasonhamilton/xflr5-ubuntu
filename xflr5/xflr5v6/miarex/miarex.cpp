...
void Miarex::onDefineWPolarHeadless(WPolar* pNewWPolar, Plane* plane)
{
    ...
    if (true)
    {
        ...
        {           
            pNewWPolar->setReferenceSpanLength(m_pCurPlane->planformSpan());
            double area = plane->planformArea(pNewWPolar->bIncludeWing2Area());
            if(plane && plane->biPlane()) area += plane->wing2()->planformArea();
        ...
        {            
            ...
            double area = plane->projectedArea(pNewWPolar->bIncludeWing2Area());
            if(plane && plane->biPlane()) area += plane->wing2()->projectedArea();

            pNewWPolar->setReferenceArea(area);
        }

        // chord wasn't set in the original method, because it's mac by default. so added this line her
        pNewWPolar->setReferenceMAC(plane->mac());
